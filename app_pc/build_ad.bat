...
if exist "dist\AIChecker.exe" (
    echo.
    echo ================================================================================
    echo BUILD SUCCESSFUL!
    echo ================================================================================
    for %%I in ("dist\AIChecker.exe") do set SIZE=%%~zI
    set /a SIZE_MB=%SIZE%/1048576
    echo File: dist\AIChecker.exe
    echo Size: %SIZE_MB% MB
    echo.
    set /p RUN="Launch now? (y/n): "
    if /i "%RUN%"=="y" start "" "dist\AIChecker.exe"
) else (
    echo.
    echo ✗ Still failed. Trying console build to see errors...
    pyinstaller --onefile --console --name="AIChecker-debug" --collect-all="PySide6" final.py
    echo.
    echo Run this to see the error:
    echo   dist\AIChecker-debug.exe
)
